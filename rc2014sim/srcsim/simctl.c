 ...
 * Copyright (C) 2016 by Scott Lawrence (RC2014 sim)
 *
 * History:
 ...
 *
 * 05-MAY-16         1.27r RC2014 support added (Scott Lawrence)
 */
...
int load_rom( const char * filename )
{
	...
	if( fd == -1 ) {
		fprintf( stderr, "%s: Can't open ROM file for reading.\n", filename );
		perror("");
		return 1;
	}

	nr = read( fd, ram, 64 * 1024 );
